                                ...
                                const std::array<int, 3>& dirIsNeg) const
{
    ...
    Vector3f distance_Min = pMin - ray.origin;
    Vector3f distance_Max = pMax - ray.origin;
    
    //  * essentially, a swap op
    Vector3f DisWithDir_Min = Vector3f(
        distance_Min.x*(dirIsNeg[0]) + distance_Max.x*(1-dirIsNeg[0]),
        distance_Min.y*(dirIsNeg[1]) + distance_Max.y*(1-dirIsNeg[1]),
        distance_Min.z*(dirIsNeg[2]) + distance_Max.z*(1-dirIsNeg[2])
    );
