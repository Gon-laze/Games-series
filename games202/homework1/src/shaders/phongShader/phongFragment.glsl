...
// add by Gon laze
#define LIGHT_WIDTH 0.2   // default val in DirectionalLight.js: just use one-face-areaSize 
...
float findBlocker( sampler2D shadowMap,  vec2 uv, float zReceiver ) {
  ...
  // //   if (sampleZ < zReceiver-EPS)    continue;
  // //   else                            {  blockerAvgZ += sampleZ;  totalWeight += 1.0;  }
...
float PCF(sampler2D shadowMap, vec4 coords) {
  ...
  for (int i=0; i<PCF_NUM_SAMPLES; i++)
    shadowVal += ((unpack(texture2D(shadowMap, coords.xy + poissonDisk[i] * filterSize)) < coords.z) ? 0.0 : 1.0);
...
float useShadowMap(sampler2D shadowMap, vec4 shadowCoord){
  ...
  return ((unpack(texture2D(shadowMap, shadowCoord.xy)) + EPS  < shadowCoord.z) ? 0.0 : 1.0);
  // return 1.0;
}
